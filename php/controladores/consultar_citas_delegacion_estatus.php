...
if (isset($_GET['bandera']) && $_GET['bandera'] == 1) {
    ...
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND Citas.estatusCitaId = 1 AND Delegaciones.delegacionId = '$delegacionId'";
    ...
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND Citas.estatusCitaId = 1 AND Delegaciones.delegacionId = '$delegacionId' ORDER BY Citas.estatusCitaId ASC LIMIT  $registro_inicial, $registros_por_pagina";
    $resultCitas = mysqli_query($conexion, $sql);

    $idcheck = 'checkRegistro';
...
if (isset($_GET['bandera']) && $_GET['bandera'] == 2) {
    $consulta = "SELECT count(*) AS conteo FROM Citas, Delegaciones, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId 
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND Citas.estatusCitaId = 2 AND Delegaciones.delegacionId = '$delegacionId'";
    $resultado = mysqli_query($conexion, $consulta);
    $consulta = mysqli_fetch_array($resultado);
    $total_registros = $consulta['conteo'];
    $total_paginas = ceil($total_registros / $registros_por_pagina);
    ...
    $sql = "SELECT Citas.citaId, Citas.fechaRegistro, Citas.fechaCita, IFNULL(Citas.estatusPlataId, 0) estatusPlataId, IFNULL(Citas.reportePlataId, 0) reportePlataId, Catalogo_Marca.descripcion, Citas.estatusCitaId, Vehiculos.numeroSerie, Vehiculos.numeroPlacas, 
    Vehiculos.numeroMotor FROM Citas, Delegaciones, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId 
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND Citas.estatusCitaId = 2 AND Delegaciones.delegacionId = '$delegacionId' ORDER BY Citas.estatusCitaId ASC LIMIT  $registro_inicial, $registros_por_pagina";
    $resultCitas = mysqli_query($conexion, $sql);

    $idcheck = 'checkPresentacion';
...
//ESTADO TRÁMITE
if (isset($_GET['bandera']) && $_GET['bandera'] == 3) {
    $consulta = "SELECT count(*) AS conteo FROM Citas, Delegaciones, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId 
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND Citas.estatusCitaId = 3 AND Delegaciones.delegacionId = '$delegacionId'";
    $resultado = mysqli_query($conexion, $consulta);
    $consulta = mysqli_fetch_array($resultado);
    $total_registros = $consulta['conteo'];
    $total_paginas = ceil($total_registros / $registros_por_pagina);
    ...
    $sql = "SELECT Citas.citaId, Citas.fechaRegistro, Citas.fechaCita, IFNULL(Citas.estatusPlataId, 0) estatusPlataId, IFNULL(Citas.reportePlataId, 0) reportePlataId, Catalogo_Marca.descripcion, Citas.estatusCitaId, Vehiculos.numeroSerie, Vehiculos.numeroPlacas, 
    Vehiculos.numeroMotor FROM Citas, Delegaciones, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId 
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND Citas.estatusCitaId = 3 AND Delegaciones.delegacionId = '$delegacionId' ORDER BY Citas.estatusCitaId ASC LIMIT  $registro_inicial, $registros_por_pagina";
    $resultCitas = mysqli_query($conexion, $sql);

    $idcheck = 'checkTramite';
...
if (isset($_GET['bandera']) && $_GET['bandera'] == 4) {
    $consulta = "SELECT count(*) AS conteo FROM Citas, Delegaciones, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId 
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND Citas.estatusCitaId = 4 AND Delegaciones.delegacionId = '$delegacionId'";
    $resultado = mysqli_query($conexion, $consulta);
    $consulta = mysqli_fetch_array($resultado);
    $total_registros = $consulta['conteo'];
    $total_paginas = ceil($total_registros / $registros_por_pagina);
    ...
    $sql = "SELECT Citas.citaId, Citas.fechaRegistro, Citas.fechaCita, IFNULL(Citas.estatusPlataId, 0) estatusPlataId, IFNULL(Citas.reportePlataId, 0) reportePlataId, Catalogo_Marca.descripcion, Citas.estatusCitaId, Vehiculos.numeroSerie, Vehiculos.numeroPlacas, 
    Vehiculos.numeroMotor FROM Citas, Delegaciones, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId 
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND Citas.estatusCitaId = 4 AND Delegaciones.delegacionId = '$delegacionId' ORDER BY Citas.estatusCitaId ASC LIMIT  $registro_inicial, $registros_por_pagina";
    $resultCitas = mysqli_query($conexion, $sql);

    $idcheck = 'checkConcluido';

    //OBTENER LA CANTIDAD DE REGISTROS A MOSTRAR EN LA PÁGINA
    $consultarRegistros = "SELECT COUNT(*) AS totalRegistros FROM Citas, Delegaciones, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId 
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND Citas.estatusCitaId = 4 AND Delegaciones.delegacionId = '$delegacionId' ORDER BY Citas.estatusCitaId ASC LIMIT  $registro_inicial, $registros_por_pagina";
    $resultRegistros = mysqli_query($conexion, $consultarRegistros);
    $row = mysqli_fetch_assoc($resultRegistros);
...
if (isset($_GET['bandera']) && $_GET['bandera'] == 5) {
    $consulta = "SELECT count(*) AS conteo FROM Citas, Delegaciones, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId 
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND (Citas.estatusCitaId = 16 OR Citas.estatusCitaId = 17) AND Delegaciones.delegacionId = '$delegacionId'";
    $resultado = mysqli_query($conexion, $consulta);
    $consulta = mysqli_fetch_array($resultado);
    $total_registros = $consulta['conteo'];
    $total_paginas = ceil($total_registros / $registros_por_pagina);
    ...
    $sql = "SELECT Citas.citaId, Citas.fechaRegistro, Citas.fechaCita, IFNULL(Citas.estatusPlataId, 0) estatusPlataId, IFNULL(Citas.reportePlataId, 0) reportePlataId, Catalogo_Marca.descripcion, Citas.estatusCitaId, Vehiculos.numeroSerie, Vehiculos.numeroPlacas, 
    Vehiculos.numeroMotor FROM Citas, Delegaciones, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId 
    AND Citas.delegacionId = Delegaciones.delegacionId AND DATE(Citas.fechaCita) = '$fechaHoy' AND (Citas.estatusCitaId = 16 OR Citas.estatusCitaId = 17) AND Delegaciones.delegacionId = '$delegacionId' ORDER BY Citas.estatusCitaId ASC LIMIT  $registro_inicial, $registros_por_pagina";
    $resultCitas = mysqli_query($conexion, $sql);

    $idcheck = 'checkCanceladas';
...
if ($resultCitas) {
    ...
    if ($pagina_actual > 1) {
        ...
        echo '<a class="page-link" href="citas_delegaciones_cp.php?pagina=' . ($pagina_actual - 1) . '&idcheck=' . $idcheck . '" aria-label="Previous">
    ...
    for ($i = 1; $i <= $total_paginas; $i++) {
        if ($pagina_actual == $i) {
            echo '<li class="page-item active"><a class="page-link" href="citas_delegaciones_cp.php?pagina=' . $i . '&idcheck=' . $idcheck . '">' . $i . '</a></li>';
        } else {
            echo '<li class="page-item"><a class="page-link" href="citas_delegaciones_cp.php?pagina=' . $i . '&idcheck=' . $idcheck . '">' . $i . '</a></li>';
        }
    }
