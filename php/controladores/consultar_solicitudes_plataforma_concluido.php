...
//VARIABLE PARA SABER QUE CHECK DEBEMOS MARCAR
$check = 'checkConcluido';
...
$consulta = "SELECT count(*) AS conteo FROM Citas, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId AND
    Citas.estatusPlataId != 'NULL' AND DATE(Citas.fecha_sol_plata) = '$fechaHoy' AND Citas.estatusPlataId = 3";
...
AND Citas.estatusPlataId != 'NULL' AND DATE(Citas.fecha_sol_plata) = '$fechaHoy' AND Citas.estatusPlataId = 3 ORDER BY Citas.fecha_sol_plata DESC LIMIT $registro_inicial, $registros_por_pagina";
...
if ($resultCitas) {
    ...
    echo '<ul class="pagination justify-content-center">';
     ...
     if ($pagina_actual > 1) {
        ...
        echo '<a class="page-link" href="gestion_solicitudes_plataforma.php?pagina=' . ($pagina_actual - 1) . '&check=' . $check . '" aria-label="Previous">
    ...
    for ($i = $numeroInicio; $i <= $numeroFin; $i++) {
        if ($pagina_actual == $i) {
            echo '<li class="page-item active"><a class="page-link" href="gestion_solicitudes_plataforma.php?pagina=' . $i . '&check=' . $check . '">' . $i . '</a></li>';
        } else {
            echo '<li class="page-item"><a class="page-link" href="gestion_solicitudes_plataforma.php?pagina=' . $i . '&check=' . $check . '">' . $i . '</a></li>';
        }
    }
