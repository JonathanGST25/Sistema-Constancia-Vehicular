...
// EFINE LA CANTIDAD DE REGISTROS POR PÁGINA
$registros_por_pagina = 10;

//DEFINE LA CANTIDAD DE REGISTROS EN LA PÁGINA ACTUAL
$total_reg_pag_act = 0;

//VARIABLE PARA ALMACENAR LA FECHA DE SOLICITUD
$fechaSolicitud = "";

//EN CASO DE BUSQUEDA POR CARACTERISTICAS DEL VEHICULO
if (isset($_GET['fechaSolicitud']) && $_GET['fechaSolicitud'] != '') {
    $fechaSolicitud = $_GET['fechaSolicitud'];
    $consulta = "SELECT count(*) AS conteo FROM Citas, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = Vehiculos.marcaId
    AND date(Citas.fecha_sol_plata) = '$fechaSolicitud' AND Citas.estatusPlataId != 'NULL'";
    ...
    $sql = "SELECT Citas.citaId, Citas.fecha_sol_plata, Citas.fechaCita, Citas.estatusPlataId, IFNULL(Citas.reportePlataId, 0) reportePlataId, Catalogo_Marca.descripcion, Citas.estatusCitaId,
    Vehiculos.numeroSerie, Vehiculos.numeroPlacas, Vehiculos.numeroMotor FROM Citas, Vehiculos, Catalogo_Marca WHERE Vehiculos.vehiculoId = Citas.vehiculoId AND Catalogo_Marca.marcaId = 
    Vehiculos.marcaId AND  date(Citas.fecha_sol_plata) = '$fechaSolicitud' AND Citas.estatusPlataId != 'NULL' ORDER BY Citas.fecha_sol_plata DESC LIMIT  $registro_inicial, $registros_por_pagina";
...
if ($resultCitas) {
    ...
    if ($pagina_actual > 1) {
        ...
        echo '<a class="page-link" href="gestion_solicitudes_plataforma.php?fechaSolicitud=' . $fechaSolicitud . '&pagina=' . ($pagina_actual - 1) . '" aria-label="Previous">
    ...
    for ($i = $numeroInicio; $i <= $numeroFin; $i++) {
        if ($pagina_actual == $i) {
            echo '<li class="page-item active"><a class="page-link" href="gestion_solicitudes_plataforma.php?fechaSolicitud=' . $fechaSolicitud . '&pagina=' . $i . '">' . $i . '</a></li>';
        } else {
            echo '<li class="page-item"><a class="page-link" href="gestion_solicitudes_plataforma.php?fechaSolicitud=' . $fechaSolicitud . '&pagina=' . $i . '">' . $i . '</a></li>';
        }
    }
